...
public class EditCommentActivity extends ThemeActivity {

    Context cntxt;
    String lang, postID, commentID, myIDtoString, comment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!sharedPrefMngr.loggedIn()){
            finish();
            startActivity(new Intent(EditCommentActivity.this, LoginAct.class));
            return;
        }
        ...
        activityInd = userParams.getInt("activityInd");
    ...
    private void saveEdit() {
        try {
            ...
            if(!StringUtils.isEmpty(comTxt)) {
                ...
                if(activityInd == 0)
                    CommentsActivity.saveEdition(cntxt, comTxt, commentID);
                else if(activityInd == 1)
                    RepliesActivity.saveEdition(cntxt, comTxt);
                else if(activityInd == 2)
                    PostDisplayAct.saveEdition(cntxt, comTxt, commentID);
                finish();
            }
        } catch (JSONException e) {
            e.printStackTrace();
