 ...
 */
public class PageVideosFragment extends Fragment {

    GridView gridView;
    ImageAdaptor imageAdaptor;
    LinearLayout layout;
    Context cntxt;
    ArrayList<String> files = new ArrayList<>(), selectedDatas = new ArrayList<>(), loadedArr = new ArrayList<>(), videosArr = new ArrayList<>();
    boolean loadingPost = false, allLoaded = false, firstLoad = true, loadedVideos;
    int imgW, gridViewPos, pageId, width;

    public PageVideosFragment(ContextData contextData) {
        // Required empty public constructor
        cntxt = contextData.context;
        pageId = contextData.dataId;
        width = contextData.width;
        loadedVideos = contextData.loadedPhotos;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        int w = width - 5;
        imgW = (w / 3) - 10;
        View view = inflater.inflate(R.layout.fragment_page_videos, container, false);
    ...
    public void getFileDisplay(){
        ...
        try {
            RequestBody requestBody = new MultipartBody.Builder().setType(MultipartBody.FORM)
                    .addFormDataPart("dataId", String.valueOf(pageId))
                    .addFormDataPart("dataType", "page")
